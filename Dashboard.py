...
st.markdown("""
             <h1 style="color:#3aa1a2;font-size:2.3em;font-style:italic;font-weight:700;margin:0px;">
                ...
                """, 
                    unsafe_allow_html=True)
st.write("")
st.write(f"Genre : **{lecture_X_test_original()['code_gender'].values[0]}**")
st.write(f"Tranche d'âge : **{lecture_X_test_original()['age_client'].values[0]}**")
st.write(f"Ancienneté de la piède d'identité : **{lecture_X_test_original()['anciennete_cni'].values[0]}**")
st.write(f"Situation familiale : **{lecture_X_test_original()['name_family_status'].values[0]}**")
st.write(f"Taille de la famille : **{lecture_X_test_original()['taille_famille'].values[0]}**")
st.write(f"Nombre d'enfants : **{lecture_X_test_original()['nbr_enfants'].values[0]}**")
st.write(f"Niveau d'éducation : **{lecture_X_test_original()['name_education_type'].values[0]}**")
st.write(f"Revenu Total Annuel : **{lecture_X_test_original()['total_revenus'].values[0]} $**")
st.write(f"Type d'emploi : **{lecture_X_test_original()['name_income_type'].values[0]}**")
st.write(f"Ancienneté dans son entreprise actuelle : **{lecture_X_test_original()['anciennete_entreprise'].values[0]}**")
st.write(f"Type d'habitation : **{lecture_X_test_original()['name_housing_type'].values[0]}**")
st.write(f"Densité de la Population de la région où vit le client : **{lecture_X_test_original()['pop_region'].values[0]}**")
st.write(f"Evaluations de *'Prêt à dépenser'* de la région où vit le client : \
                   **{lecture_X_test_original()['region_rating_client'].values[0]}**")
    # Titre 2
st.markdown("""
                <h1 style="color:#3aa1a2;font-size:2.3em;font-style:italic;font-weight:700;margin:0px;">
...
st.markdown("""
                ...
                """, 
                unsafe_allow_html=True)
X= st.selectbox("*Veuillez sélectionner une variable à l'aide du menu déroulant 👇*", 
    (lecture_X_test_clean().drop(labels="sk_id_curr", axis=1).columns.to_list()), 
                          key="X_tab1")
st.write('You selected:', X)
fig = px.histogram(x=lecture_X_test_clean()["sk_id_curr"], y=lecture_X_test_clean()[X], color=y_pred_lgbm_proba_df.columns,
                   marginal="box", # or violin, rug
                   hover_data=lecture_X_test_clean().drop(labels="sk_id_curr", axis=1).columns)
fig.show()